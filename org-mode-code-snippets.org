...
** Get Filename from URL in another Table Cell using ~url-file-nondirectory~ and ~org-lookup-first~
    ...
    #+BEGIN_QUOTE
      *Important:* No unquoted whitespace allowed in =Property-Name= and =Property-Value= columns because ~org-lookup-first~ function will provide incorrect answer.
    #+END_QUOTE
...
* Exporting Subtrees via Command-line

#+BEGIN_SRC org 
  ,* Export this Subtree
  :PROPERTIES:
  :EXPORT_FILE_NAME: exported-file.html
  :EXPORT_TITLE: Exported Title
  :ID:       1582d1f4-600f-4018-89c8-8dcb4c05117b
  :END:
  ...
  : https://emacs.stackexchange.com/a/48520/388

  ,#+BEGIN_SRC shell :tangle htmlexport :shebang "#!/usr/bin/env bash"
    emacs --batch --funcall package-initialize --eval "(progn (find-file \"$1\") \
                         (org-id-goto \"$2\") \
  ...
  ,#+BEGIN_SRC shell
  ./htmlexport ./junk33.org '1582d1f4-600f-4018-89c8-8dcb4c05117b'
  ,#+END_SRC
#+END_SRC

...
To update list need to pass `X-RequestDigest: FormDigestValue` to Get FormDigestValue submit `POST` to `/_api/contextinfo` with no data. See exampple curl command below:

...
#+header: :var data='nil
#+header: :var filter='nil
#+header: :var columns='nil
...
#+begin_src ruby
  ...
  columns.map!{|c| data[0].index(c) ? data[0].index(c) : Integer(c) rescue nil }.select{ |c| c }
  
  # Feature: Logical OR filter array
  data.select{|d| filter.empty? ? true : filter.map{|f| d.include?(f) }.inject(&:|) }.map{|d1| Array.new(columns.length){|i| d1.fetch(columns[i])}}
#+end_src
